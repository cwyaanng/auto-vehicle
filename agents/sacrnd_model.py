...
from stable_baselines3.common.utils import polyak_update # 타겟 네트워크 파라미터 소프트 업데이트 [의문]
...
class SACOfflineOnline(SAC): # SAC 상속한 커스텀 에이전트 
    ...
    def prefill_from_npz_folder(self, data_dir: str, clip_actions: bool = True) -> int: # 오프라인 파일들을 버퍼 안에 집어넣기 
    ...
    # critic을 고정한 상태에서 actor만 학습 
    def pretrain_actor(self, steps: int = 3000) -> None:

        for p in self.policy.critic.parameters():
            p.requires_grad = False
        self.policy.critic.eval()
        self.policy.actor.train()
        # 리플레이 버퍼에서 배치 샘플을 꺼낸다 
        for _ in range(steps):
            batch = self.replay_buffer.sample(self.batch_size, env=self._vec_normalize_env)

            # 현재 actor 정책으로 행동 샘플 + 로그 확률 
            actions_pi, logp_pi = self.policy.actor.action_log_prob(batch.observations)
            # 고정된 critic을 통해 방금 샘플한 행동의 Q 값 계산 
            q1_pi, q2_pi = self.policy.critic(batch.observations, actions_pi)
            # 두 개 중 작은 값을 취해 보수적으로 평가 
            q_pi = th.min(q1_pi, q2_pi).detach()

            # 엔트로피 계수 가져옴 
            alpha = self._alpha()
            # SAC Actor 목적함수 -> 결과적으로 이 값을 최소화해야함 
            actor_loss = (alpha * logp_pi - q_pi).mean()
            if(_ % 10000 == 0) :
                print(f"[SAC&RND ACTOR pretrain] actor loss : {actor_loss}")
                
            # Actor 파라미터에 대한 역전파/최적화 수행 
        ...
        # 모든 파라미터 텐서를 순회하면서 critc을 학습 가능 상태로 바꿈 
        for p in self.policy.critic.parameters():
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
       # print(f"TRAIN BATCH SIZE : {batch_size}")
        # policy 내부 모드 설정 -> 여러 레이어들이 학습 모드로 전환됨
        self.policy.set_training_mode(True)

        # Actor/ critic optimizer 리스트 준비. 
        # ent_coef_optimzer은 온도 파라미터 학습용 
        optimizers = [self.policy.actor.optimizer, self.policy.critic.optimizer]
        if self.ent_coef_optimizer is not None:
            optimizers.append(self.ent_coef_optimizer)
        self._update_learning_rate(optimizers)

        # 손실을 기록할 리스트 초기화 -> 통계 기록용 리스트 
        ent_coef_losses, ent_coefs = [], []
        actor_losses, critic_losses = [], []

        # replay buffer 안에서 에피소드 단위로 mc return을 미리 계산해 둔다. 
        self._ensure_mc_returns_current()

        # 학습 루프 
        for gradient_step in range(gradient_steps):
            
            # replay buffer에서 미니배치 뽑음 
            global_update = self._n_updates + gradient_step
            # print(f"global step : {global_update}")
            rb = self.replay_buffer
            size = rb.buffer_size if rb.full else rb.pos
            batch_inds = np.random.randint(0, size, size=batch_size)   # <- 인덱스 확보
            replay_data = rb._get_samples(batch_inds, env=self._vec_normalize_env)  # SB3 내부 메서드

            # 랜덤 노이즈 적용 [의문]
            if self.use_sde:
                self.policy.actor.reset_noise()

            # actor이 현재 정책에 따라 행동 샘플, log probability 리턴 
            ...
            with th.no_grad():
                ...
                # RND novelty로 가중치를 주어 SAC target과 monte carlo return의 가중합 만들기 
                nov = self.rnd.novelty(replay_data.observations)  # (B,1), detach된 값

                # 러닝 통계 갱신 (전 배치 누적 통계)
                self._nov_rms.update(nov)

                # z-score -> sigmoid -> 안정적 클램프
                ...
                rnd_norm = th.sigmoid(nov_z)          # ~0~1
                rnd_norm = rnd_norm.clamp_(0.05, 0.3)  # 5%~95%로 가중치 과도치 방지

                idx = batch_inds
                mc_batch = th.as_tensor(self._mc_returns[idx], device=self.device).view(-1, 1)
                if gradient_step % 200 == 0:  # 너무 자주 찍지 않기
                    def tstat(name, t):
                        t_ = t.detach().to("cpu").view(-1)
                        print(f"[{name}] shape={tuple(t.shape)} "
                            f"min={t_.min().item():.4f} max={t_.max().item():.4f} "
                            f"mean={t_.mean().item():.4f} std={t_.std(unbiased=False).item():.4f}")

                    # NaN/Inf 빠르게 검출
                    for nm, tt in [("nov", nov), ("rnd_norm", rnd_norm),
                                ("target_q_sac", target_q_sac), ("mc_batch", mc_batch)]:
                        if th.isnan(tt).any() or th.isinf(tt).any():
                            raise ValueError(f"[NaN/Inf] {nm}")

                    # 통계
                    tstat("rnd_norm", rnd_norm)
                    tstat("target_q_sac", target_q_sac)
                    tstat("mc_batch", mc_batch)

                    # 가중치 합이 1인지, 범위가 맞는지
                    w = rnd_norm.detach().to("cpu")
                    one_minus_w = (1 - w)
                    print(f"[w sum check] min w={w.min().item():.4f}, max w={w.max().item():.4f}, "
                        f"min(1-w)={one_minus_w.min().item():.4f}, max(1-w)={one_minus_w.max().item():.4f}")

                    # 몇 샘플만 상세 확인
                    idxs = [0, 1, 2]
                    for i in idxs:
                        w_i   = rnd_norm[i].item()
                        sac_i = target_q_sac[i].item()
                        mc_i  = mc_batch[i].item()
                        mix_i = ((1 - w_i) * sac_i + w_i * mc_i)
                        print(f"[sample {i}] w={w_i:.3f} sac={sac_i:.3f} mc={mc_i:.3f} mix(calc)={mix_i:.3f}")
                
                target_q_values = (1.0 - rnd_norm) * target_q_sac + rnd_norm * mc_batch
                
                # if(gradient_step % 100 == 0):
                #     print(f"agents/sacrnd_model.py target_q_values : {target_q_values} = target_q_sac {target_q_sac} + rnd_norm {rnd_norm} * mc_matched {mc_matched}")
            ...
            # actor 업데이트 
            q_values_pi = th.cat(self.policy.critic(replay_data.observations, actions_pi), dim=1)
    ...
    def online_learn(self, total_timesteps=300_000, tb_log_name: str = None, log_interval: int = 10, callback=None):
